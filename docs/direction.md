...
Both still support the reading below, which is about density and openness.
`journaling-under-tree` was later held back from teaching on a different
axis — its line reads sketched rather than printed (D92) — so it measures
§3.5 without being an exemplar.
...
| D8 | No auth, no `users` table. Reaffirmed 2026-09-04: a team of one. |
| D9 | Every job records resolved prompt, model, params, usage and cost. |
| D10 | Print pipeline planned from the start, built at ~~Stage E~~ **Stage F** — §7 renumbered the stages and E is Batch. Print is built; Batch is not. |
...
| D17 | Page type is the template unit. ~~Six templates for coloring books~~ → **seven are active**. The six `hs-*` page-type templates plus `adult-coloring-clean-line`, the Stage 1 original, which carries **no `page_type` at all**. That is not just a miscount: D56 refuses an item whose page type disagrees with its template, and the check cannot fire against a template with nothing to disagree with. It is a live bypass around a guard, and not dead — it generated the asset now registered as a style reference (D91). Either give it a page type or retire it. |
...
| D21 | **Retired, never implemented.** See D90. |
| D22 | Approved pages can be promoted to style references. Three gates, and enforced since 029; see D91. |
...
| D26 | **Art Style is a first-class dimension**, separate from category and tone. ~~Twelve values (§4.4)~~ → **per category**, see D96. This is the mechanism for variety; without it the tool has no axis to vary. |
...
| D29 | Output sizes are fixed per category (§5.1). **Landscape 1536×1024 is never used** — enforced by the `categories` size CHECK, and the dead rate row is gone from `lib/pricing.ts`. Open for Stage G: social-content is square-only, see D93. |
...
| D32 | **VV-Styles moves to Stage C**, ahead of library and batch. ~~and print~~ — Stage F was built first, so print no longer sits behind it. It is the simplest complete path through the engine and the store needs product before launch. |
...
| D54 | **Reopened — the wiring landed, the data did not. See D97.** **`items.visual_elements` feeds the `{{environment}}` slot.** Nothing supplied it, so the one block that says what is in the room was silently dropped from every item-driven generation — an empty slot drops its block by design. Generic rooms were not the model guessing badly; they were the item never being asked. |
...
| D57 | **Partly superseded.** `quote_text` does go in the prompt for apparel (D70), and the export stage now exists (D97). The rule below holds for coloring pages only. **`quote_text`, `quote_lang` and `lettering_style` are deliberately not prompt inputs.** D23 puts quote text on the page as overlaid outlined vector type; sending it to the model would produce misspelt, uncolourable lettering and break the reserved space the composition block creates. They are export-stage data, and the export stage does not exist yet. `color_direction` and `product_placement` are VV-Styles fields with no VV-Styles template (D32, Stage C). **Five of the six unused columns are unused correctly** — only `visual_elements` was a genuine defect. |
...
| D68 | **Every lettering gets its own storage key.** D65 protects the art from being replaced but said nothing about the letterings, and the key was derived from the source page — so each re-letter overwrote the previous one's PNG and SVG while its asset row survived, pointing at a file that now held different words. What that broke is narrower than first recorded, and the correction matters. The print does **not** read the stored SVG — it re-lays the type from `overlay.text`, `area` and `strokeWidth` on the asset's own row (D99, `export/[id]/route.ts:187`), all of which are per-row and cannot collide. So the PDF would have printed the right words even before the fix; what went wrong was the **preview PNG** showing a different quote than the row it belonged to. Unique keys are still correct — a row must not point at a file holding someone else's words — but the original justification described a print pipeline that reads stored SVGs, and that pipeline does not exist. |
...
| D65 | **Lettering a page creates a new asset; it never replaces the original.** The unlettered art stays the artwork, so a quote can be reset, restyled or translated against it any number of times. The SVG is written beside the PNG as a record of what was drawn — **not** as the print source, per D99, which corrected the same sentence in D68 and missed this one. What goes to print. |
...
| D48 | **Superseded by D92.** It was also never accurate: at the time only `hoodie-on-sofa-v2` carried `usable_as_input = true`, and `journaling-under-tree` is now held back as a study because its line reads sketched rather than printed. The canonical Solo portrait exemplars are **`walking-with-mug` and `hoodie-on-sofa, corrected`**. Both are Esoh's own work, so D31 does not apply — that part stands. |
...
| D37 | **Every live product exists as a library row.** ~~`Listenin' for the GOOD E.S.H.` is currently live but unrecorded.~~ → recorded as VVS-0130. |
...
| D89 | **Localhost-first stays; it stops meaning one copy.** D7 costs nothing — one user (D1), no unattended work (D87), nothing external needing to reach in — and hosting would buy auth, deploys and a bill for a tool only Esoh opens. What was underpriced is D15's "container files are not backed up". `/storage` was gitignored entirely: 90 MB, 40 files, every image ever paid for, in one place. The dollars are not the loss. `reference_images` points at four files — `journaling-under-tree`, `walking-with-mug`, `hoodie-on-sofa` and its correction — and §6 holds that the Style Library cannot carry a style until one fully approved page exists. **Those four files are that.** Losing them does not mean re-running a generation, it means re-deriving the house style. Second and quieter: the 4 approved and 12 rejected judgments live only in local Postgres, and nothing else records which way a page was called. Two fixes, both small. The exemplars are committed — `.gitignore` now excepts `/storage/exemplars`, since they change almost never and `direction.md` already cites them by name. And `npm run db:dump` writes a timestamped `pg_dump` to `/backups`, gitignored, because a copy beside the original is not a backup. **The chain that follows from this being a Crostini container on a Chromebook:** `/backups` covers a bad migration; `/mnt/chromeos/MyFiles/Esoh Backups/Esoh-studio backups` covers the Linux environment being reset, which is the likely failure and the one the container cannot cover for itself; Google Drive covers the Chromebook; GitHub covers code and exemplars. Done 2026-09-04 — dump and exemplars are on the ChromeOS side and copied to Drive. Neither changes D7. |
| D90 | **D21 is retired rather than implemented: no `/api/v1`, no `{data, meta, error}` envelope.** Nine routes exist and not one is versioned or enveloped — they return `{ assets }`, `{ books }`, `{ jobId, prompt, assets }` and `{ error }` — so the decision had been describing an API that does not exist. Versioning exists to let an API change without breaking clients its authors do not control; this API has one client, in the same repository, deployed atomically with it. The envelope buys a little more — one uniform error path instead of an ad hoc `if (!d.error)` at each call site — but not nine routes and every fetch in `page.tsx` rewritten to gain a wrapper key. Retiring it costs nothing and stops the document asserting a shape the code never had. |
| D91 | **D22's promotion path had never been walked, and is now enforced.** `reference_images.asset_id` has existed since the start to record which approved page a reference came from. All four rows carried NULL — including the only row with `usable_as_input = true`, `Hoodie on sofa, corrected`, which steers every Solo portrait generation and got there by being copied into `storage/exemplars/`. All four approved assets sat unpromoted. Promotion is deliberately **three gates, not one**: approve the image, promote it to a reference, then clear it for input — `usable_as_input` defaults false per D31. Approval says the page is good; promotion says it should teach the next one, and a page can be good and a poor teacher. Migration 029 records the one provable link (`journaling-under-tree.png` is byte-identical to asset `608ce084`, still `draft` — approving it is Esoh's call) and adds two triggers: a reference naming an asset must name an **approved** one, and an asset already teaching cannot leave approved without the reference being stood down first. Hand-placed seeds keep working, since they predate the tool and have no asset to name. The failure being closed is the compounding one — a bad reference does not spoil one page, it teaches every page after it. |
| D92 | **Line quality is a separate axis from density, and a page can pass one while failing the other.** Two references were promoted to exemplars on 2026-09-04 — `Walking with mug` and `Hoodie on sofa, corrected` — and one was deliberately held back: `Journaling under a tree` reads sketched and hand-drawn rather than confident and printed. That page is one of the two §3.5 measured the density correction against, and the correction stands: it is open, its background is countable shapes, it breathes. It fails on something §3.5 never looked at. **Density asks how much of the page carries subject matter; line quality asks whether the mark looks drawn once with certainty or felt out.** A style block already asks for "confident heavy contours" (§3.5), so the vocabulary exists — what was missing was the recognition that the two tests are independent, and that a page can be promoted on one and refused on the other. Held-back pages stay `kind = 'study'`: still evidence for how templates are written, never sent to the model. Two exemplars now feed Solo portrait generation instead of one. |
| D93 | **Open for Stage G: social-content is square-only and probably should not be.** The `categories` CHECK permits 1024×1024 and 1024×1536 (2:3), and social-content is set to the square. Instagram's feed favours 4:5 and stories and reels are 9:16, so a square-only category is cropped or letterboxed nearly everywhere it lands. Neither permitted shape is 4:5. This is not a bug today — Stage G has not started and no social content has been generated — but it is a decision waiting there, and it touches the CHECK constraint rather than a settings row, so it is not a one-line change when it comes. `print-designs` and `social-content` also carry no `deliver_width`/`deliver_height` at all, where coloring-books has 2550×3300 and vv-styles 3600×4800. |
| D94 | **Print areas are read from Printify, and D71's 12×16" was wrong for every product.** `print_areas` was built at Stage F and never populated — the script needed a token that was not documented until 2026-09-04. Run against the live shop it returns **16 areas across 5 blueprint/provider pairs**, and the front areas are not close to each other: Heavy Cotton **13.2×16.4"**, Ultra Cotton and Softstyle **10.6×12.1"**, Jersey **9.2×11.2"**, Tumbler **9.3×7.0"**. A first run also picked up an Oversized Boxy Tee at 10.7×12.2", which turned out to exist only in a **disconnected** shop — Printify keeps those in the API with their products intact, so the script now skips any shop whose `sales_channel` is `disconnected`. **Consequence worth acting on: the oversized tee is not in the live shop, so the tool has no dimensions for it.** The Jersey front is less than half the printable area of the Heavy Cotton front. D71's hardcoded 12×16 matched none of them. `vv-styles` now delivers into **3951×4919**, the Heavy Cotton front, read rather than assumed — master at the largest and let Printify scale down. A second mismatch is not fixed by this: generated art is 1024×1536, a ratio of **0.67**, while every shirt front runs **0.80–0.88**. Padded into one, a design lands tall and narrow with wide empty margins on both sides, which is a composition problem and not a size setting. The five tee blueprints all come from print provider 99, so provider variation is not currently biting — blueprint variation is, and it is large. |
| D95 | **Open: the Tumbler needs landscape, and landscape is forbidden.** The shop sells a Tumbler 20oz whose front print area is **2795×2100 — 9.3×7.0", landscape**. D29 says landscape is never used and the `categories` size CHECK makes 1536×1024 unconfigurable, so no category in this tool can generate art for a product already being sold. `vv-styles` is 1024×1536 portrait and all 19 areas including the tumbler's were filed under it, which is wrong on its face — one category cannot deliver into both a portrait shirt front and a landscape tumbler wrap. Not urgent: tumbler art is presently made by hand in Printify's editor like everything else was. But it is the second product-shaped hole after D93's social ratios, and both point the same way — the size CHECK encodes an assumption about output that the product range has already outgrown. |
| D96 | **Art styles are per category, and there are 16 across two of them — not twelve.** D26 named twelve values and D36 called Photoreal Composite a thirteenth; neither number matches the database, because a single list was never the right shape. `prompt_blocks.art_style` is keyed by category and always was. **coloring-books carries 7** — Bold Minimal, Botanical Line, Celestial, Editorial Scene, Geometric Abstract, Hand-Drawn Doodle, Zentangle Pattern. **vv-styles carries 9** — Bold Minimal, Editorial Typographic, Hand-Drawn Doodle, Photoreal Composite, Retro Comic, Retro Groovy, Streetwear Graffiti, Tattoo Linework, Vintage Badge. Two overlap by name and are separate block sets. A shirt menu and a coloring-page menu are different menus; counting them as one total is what produced three different numbers in three places. Only 10 of the 16 currently appear on any item, all of them apparel — the coloring styles exist as blocks awaiting the 180 rows that have no `art_style` set. |
| D97 | **Open: `{{environment}}` is wired and starving, and the fix may be the wrong one.** D54 diagnosed generic rooms as "the item never being asked" and wired `items.visual_elements` into the `{{environment}}` slot. The wiring is correct (`app/api/generate/route.ts:154`). The data is not there: **1 of 180 coloring-book items has `visual_elements`**, and 12 of 130 apparel items. So `hs-environment` is still dropped for 179 of 180 pages, exactly as before the fix. But the briefs say the diagnosis was slightly off — "Woman laughing softly on shaded porch", "Small support circle in a bright community room" — **the setting is already in the brief**, and `brief` feeds `{{subject}}`. The environment is not missing; it is answering in a different slot than the one asking. Splitting 180 briefs into subject and setting is D78-shaped drafting work with an unproven payoff, since a separate `hs-environment` sentence may add nothing the subject line has not already said. **Test on one page before doing 180.** |
| D98 | **A missing glyph fails loudly instead of printing an empty box.** `fontkit` answers an unsupported character with glyph 0, `.notdef`, and draws nothing. Nothing throws, so a quote containing one character the face lacks produced a page that looked finished and was not — the exact failure the project keeps meeting, where every number passes and the picture is wrong. `layoutQuote` now checks coverage before laying anything out and names the characters the face cannot set. D19 commits Quote pages to accented Spanish and the faces clear it — `Sanación no es una línea recta` sets clean on Hand-Marker — but the margin is one language wide: Permanent Marker carries 229 glyphs and its own note in `lettering_faces` reads "Spanish and no further". The check costs one pass over text the layout was about to walk anyway. |
| D99 | **The print re-lays the type; the stored SVG is a receipt, not the print source.** Clarifies D61, which is otherwise implemented as written. The overlay saves an `.svg` beside the raster art, but `lib/print.ts` does not embed it — it regenerates the paths at print dimensions from the recorded `area` and `strokeWidth`. That is the better half of D61 and easy to mistake for waste: the saved file was laid against a 1024-wide page, so printing it would mean scaling geometry that can be regenerated exact instead. Written down because the "optimisation" of embedding the stored file would silently give back the resolution independence D61 exists to buy. |
| D100 | **D59 is a rule awaiting a second product, and the font list is a coloring-page list.** `lettering_faces` is keyed by `lettering_style` alone, with no product dimension, and its rows carry coloring-page reasoning — Playfair is recorded as rejected because its hairlines collide into bars *when outlined*, a test that only exists because D23 makes coloring-page type hollow. Apparel type is filled and drawn by the model (D70), so nothing consults this table for a shirt and nothing is currently wrong. D59's own example is the tell: Playfair is "perfectly usable on a shirt" and is not in the table to use. **No change until apparel needs overlaid type** — at which point the table needs a product dimension, not a second opinion about the same rows. |
| D101 | **A failed knockout is said out loud, on the card, next to the picture.** D74 measured the knockout and wrote the result into `metadata_json`, where nothing read it — no filter, no badge, no query. "Flagged, never discarded" was half true: recorded, but nobody was told. The result card now carries the measurement's own words when `ok` is false, and `/api/assets` returns it so the warning survives a reload rather than living only on the card that generated it. Confirmed against the one failing asset in the library: 92% bounds fill, and Esoh's description on sight — "a cream colour between the wording and the image, like it tried to cut it out but made a sticker background" — is D72's defect in different words. **The measurement and the eye agree, which is the thing worth knowing.** Still outstanding: the grid shows twelve, so a failure older than that is invisible, and a "show me everything that failed" filter is Stage D. |
| D102 | **Rejected work loses its file and keeps its record.** 23 of 36 assets were rejected and held 56 MB of 90 MB — more disk spent on refused work than kept work, heading for roughly 1.8 GB at Stage C plus 240 pages at three attempts each, inside a Crostini container. Deleting rows was never the answer: a row carries cost (D9), the resolved prompt, lineage (D85) and the verdict, and 23 rejections are a record of what Esoh does not want that nothing else stores. Migration 030 adds `purged_at`; `npm run storage:purge` deletes the pixels and marks the row. **Dry run is the default and `--yes` is the only way past it.** Three things are never purged: approved work, anything a style reference points at (D22, D91), and any asset with derivatives — because the print re-lays quote type against the *original* art through `overlay.from` (D61, D99), so purging a parent would break a lettered child's export silently, months later. Both of the last two guards fired on real rows the first time it ran. |
| D103 | **Three settings are built and starved, and D82 was wrong about one of them.** The same shape as D97 in three more places: the mechanism exists at the block layer and no data reaches it. **Background Density** (D27) is null on **309 of 310 items** — one says Dense. **Art style** is set on 0 of the 180 coloring-book rows, so the axis D26 calls the mechanism for variety is unused on exactly the pages that must not repeat (D96). And **`{{clean_date}}`** has a block, `vvs-clean-date` — "A small ribbon within the design carries the text" — that nothing ever fills, so it drops on every generation. **The last of those corrects D82, which is withdrawn.** D82 claimed the tool had no concept of a fill-in-the-blank and that VVS-0009 was the first personalised design; D35 had already decided clean date is a prompt slot and the block was already written. What is genuinely missing is narrower: a value source, and a per-order mechanism to supply it. An empty slot drops its block by design, which is the right behaviour and also why all three of these fail silently — a prompt simply comes back shorter and nothing says why. |
| D104 | **Open: two of the three lines have no way to be drawn specifically.** §3 commits to ethnicity "rendered specifically and respectfully, never generically", and for the African American line the tool can do it: `hair` offers 23 options — twist-out, Bantu knots, Fulani braids, 360 waves, a silk press, a high-top fade — and D45 made hair item data precisely so the model would not copy the exemplar's. Every one of those 23 is an African American style. For the Hispanic and Multiracial lines the menu holds `a curly bob`, `long layered curls`, `a shaved head` and `a headwrap`, and nothing else that fits. **There is no ethnicity-aware block anywhere in the library** — a search of `prompt_blocks` for African, Hispanic, Latin, Multiracial or ethnic returns zero rows. So the entire ethnicity signal for 120 of the 180 pages is one sentence the route composes, `The figure is Hispanic.` or `The figure is Multiracial.`, and the model is left to fill in the rest from whatever it associates with the word. That is the definition of generic, and for *Multiracial* it is worse than generic — the word names no appearance at all, so the sentence asks for something unanswerable. The data is not the problem: all 60 Hispanic and 60 Multiracial briefs are written and are as specific as the African American ones. The **menus and the block layer are the problem**, and they were built while only the African American line was being generated. Raised by Esoh on 2026-09-10, unprompted, from looking at the form. |
| D105 | **A coloring page has no tone, so a colour word can only be rendered as fill.** Every area inside the outlines is white until a reader puts colour there (§3.4). A colour word in the prompt therefore names the one thing the page cannot carry, and the model resolves it the only way available — by filling the shape. Measured on `Hispanic Fall 01`, same prompt but for the word: **35.8% black in the head crop with "dark", 18.4% without**, and the waves came back with white inside them rather than reading as one mass. This is a **sibling of the law in §2.5, not a restatement**. §2.5 governs technique negatives — naming the failure summons it. This governs colour words, which have nowhere to land. Both surface as an area that should be open coming back filled, which is the likeliest reason four separate investigations walked past it. Migration 034 removes "dark" from eight briefs and eight `items.hair` values; it had to be both, because the keep-both call holds the brief as a faithful copy of the workbook, so the hair column alone would have left the word in the subject line. **Three cases are flagged rather than changed**, because each needs Esoh and not a regex: `silver-streaked hair` (Summer 08) — greying hair is age and identity, and stripping the word flattens an older woman into an unmarked one, though it is also genuinely undrawable in tone; `amber window light` (Fall 03) — asks for light on a page §3.5 established has none; and five uses of "bright", which reads as airy rather than coloured and has not been observed to fill anything. **"white space" is protected** — it is a composition term, white is what the page already is, and a broad colour sweep would have eaten it. |
| D106 | **Three fixes for filled hair failed before the fourth worked, and the exemplars were not the cause.** Recorded so the money is not spent again. **(1)** D45/032 named the hair per item — the block fired, said the right thing, hair still filled. This **corrects 014's premise**, which held that "filled hair will keep coming back for as long as nothing says what the hair is"; naming it was necessary and not sufficient. **(2)** 033 removed the technique negative from `hs-figure-rendering` — correct on its own terms under §2.5 and it did not fix this. **(3)** Generating with `useReferences` off, prompts byte-identical by MD5 — still filled, slightly flatter. That last one is the surprise: **both approved exemplars carry solid black filled hair and beards**, so pictures-beat-words was the obvious explanation and it is wrong for rendering technique. What remains true about the exemplars is a **content** claim, and it is untested: `hoodie-on-sofa-v2` has a **pumpkin on the shelf**, which is exactly what `hs-seasonal-restraint` exists to prevent, and both carry heavy leaf scatter. Two approved reference images may be teaching against a block on every page generated — worth measuring separately, and not to be confused with the rendering question this decision closes. |
| D107 | **033 and 034 verified by looking, and D106's content claim is closed: the exemplar carries the pumpkin and is not teaching it.** Three things were left unproven on 2026-09-10 and all three are now answered on the pages themselves. **(1) The window containment in 033 works.** Every post-033 page with a window — the two `Hispanic Fall 01` runs and the `Hispanic Spring 06` community scene — draws the view inside an enclosed frame with panes, and the leaves stay in it. The failure Esoh described as "leaves taped to the walls" does not recur. **(2) 034 works, and it is the fix that mattered.** Same item, same crop, the only change being the word: `36.6%` black in the head crop at 22:36 (post-033, pre-034) against `15.6%` at 23:17 (post-034). That reproduces the 35.8/18.4 of the original one-word test, and this time the hair also *reads* right — a few outlined flowing sections, white and open inside, colourable. 033 alone was not sufficient, exactly as D106 recorded. **(3) The pumpkin is real and it is not being taught.** `hoodie-on-sofa-v2` does carry an indoor pumpkin on the shelf, upper right, plus a leaf on the mug and a leaf on the candle; the v2 correction had removed five of the original's nine seasonal echoes — the framed leaf art, the leaf-stamped book, the loose couch leaves, the acorns and the pinecone — and stopped there. But no pumpkin has appeared in any page generated under `hs-seasonal-restraint`. **The block is holding against its own reference image**, so the worry D106 flagged as worth measuring is answered: no. This is the same result as the `useReferences` test in D106 pointing the same way — for both technique *and* content, the words are beating the pictures. |
| D108 | **The exemplars cover one page type of six, and the query that picks them ignores the column that would know.** `reference_images.page_type` is populated — all four rows say `Solo portrait` — and the selection in `app/api/generate/route.ts` filters on `category_id` alone, `order by created_at desc limit 3`. So a Community Scene, Environment, Symbol, Decorative or Quote page either gets two close-up single-figure portraits handed to it as its style reference, or gets nothing; the column that could tell them apart is never read. This is the D103/D104 shape a third time — a built mechanism with data sitting next to it that nothing consults — and it matters more than it looks, because a 60-page book is mostly not solo portraits. Related: **`useReferences` is not recorded on the job.** `params_json` holds `n`, `size`, `density`, `quality` and `artStyle`, and not the one flag D106 spent a controlled test on, so no past generation can be audited for whether it saw an exemplar. Both are recorded here rather than fixed, because the first needs Esoh to say whether an unmatched page type should fall back to the portrait exemplars or to none. |
| D109 | **The Multiracial line's 32 figure briefs are written, and heritage is named by origin rather than by colour.** Migration 035, the D104 data half. The Hispanic 60 could lean on a word that names something; "Multiracial" cannot, which is why that line came back unmarked. So each figure is given a heritage that is *answerable* — Nigerian and Puerto Rican, Dominican and Japanese, Cape Verdean and Swedish — and **no pairing repeats across the 32 rows**, so the line is defined by its range rather than by an invented type. That is D96 as much as D104. **No brief says "Black" or "white".** Under D105 a colour word in a subject line is an instruction to fill a shape, and those two ethnonyms would be sitting in the prompt of a page with no tone; whether the model reads them as identity or as fill is not worth finding out across 32 pages, and naming the origin is more specific anyway. For the same reason nothing describes skin — the Hispanic 60 carry their specificity on hair, age, dress and naming, 0 of 60 mention tone, and that is the half that works. **Scope is 32 rows, not 60:** of the 15 pages per season only the 4 Solo portraits and 4 Community scenes carry a figure, and the route already drops the ethnicity sentence for the other 7 (D62). Those 28 need composition detail, which is D103's work. Multiracial figure briefs now average **252 characters against Hispanic's 230**, the best-equipped rows in the library; African American is now the thin line at 47 and is the remaining half of D104. Esoh chose the combined approach — vary across the line *and* name the heritage — over either alone. |
| D110 | **Hair rendering is not stable run to run, and the black-fraction metric cannot see the difference that matters.** Four pages, same block, same template, same instruction that hair be "a few large outlined sections left white and open inside": `thick wavy hair` came back as clean flowing sections, open and colourable (15.6%); `loose spiralling curls` came back with every curl packed with fine hatching, too small to put a pencil in (13.0%); `long braids` came back as braid outlines filled with grey texture (12.5%); and a **controlled re-run of the spiral-curl page with the one phrase changed to `loose waves` came back solid filled black at 22.8%** — worse than what it replaced, and the opposite of what the wavy Hispanic page did. Two things follow. **(1) The metric is blind.** 12.5, 13.0 and 15.6 are effectively the same number across three pages whose colourability is completely different, because a head crop cannot distinguish an open section from fine hatching. Hatching is the §2.5 failure and the numbers wave it through — this is [[look at the output]] stated as a measurement fact rather than a habit. **(2) One A/B pair is not evidence here.** D105 stands: two independent pairs, 35.8→18.4 and 36.6→15.6, both on the same word. But the spread seen here means a single pair cannot settle a hair question on this project, which is a plausible reason several of the four investigations in D106 reached confident wrong conclusions. Before the next hair theory is tested, the run-to-run variance of one fixed prompt needs measuring, so it is known how many samples a claim costs. **This is the live blocker on D104's second half:** all 23 of the African American hair options are tight-texture styles — twist-out, Bantu knots, Fulani braids, 360 waves — and tight texture is what these pages render as marks inside the shape rather than as outline. |
| D111 | **The variance baseline: ten runs of one fixed prompt, and the page fails about nine times in ten.** D110 asked what a claim costs to prove. `Multiracial Fall 01`, unchanged, high quality, ten generations, $2.59. On the D105 crop the black fraction ran **17.2% to 25.7%, mean 20.8, sd 2.7** — a spread of 8.5 points with nothing changed at all. Three consequences. **(1) D105 survives, and it is one-sided.** The page carrying "dark" measured 36.6%, four points clear of the entire ten-run range and roughly six standard deviations out; that is a real effect and not noise. But the 15.6% that followed its removal sits just below a distribution whose typical member is still a filled mass. Taking the colour word out stops the prompt making things worse — it does not make the page good. **(2) The `loose waves` control in D110 was misread, and the error was mine.** 22.8% is comfortably inside the range, within one sd of the mean. The outlier was the 13.0% spiral-curl page taken as its baseline. D110 records that one A/B pair is not evidence and then the next paragraph drew a conclusion from one A/B pair; the variance run is what caught it. **(3) CORRECTED 2026-09-11 — the failure rate asserted here was not real.** This decision originally read "nine of the ten pages came back with the hair as a filled black mass" and treated that as a ~90% failure rate, which the whole of D112 was then built on. **Esoh reviewed the same ten pages and judged the hair acceptable**, and specifically noted that runs 1 and 2 "were able to be colored even with the darkness". The failure count was the model's own aesthetic judgement standing in for the product owner's, and it was wrong. What survives is narrower and still useful: the run did not test what it appeared to, because `hs-hair` was absent from all ten prompts. **`hs-hair` was not in any of those ten prompts.** `Multiracial Fall 01` has `items.hair` null, and an empty slot drops its whole block, so the dedicated hair instruction was absent and only the two general blocks — `hs-figure-rendering` and `hs-texture-as-pattern` — were speaking. Multiracial is the only line of the three with no `items.hair` anywhere, 0 of 16 Solo portraits against 16 of 16 for the other two, which is D103's silent-drop failure landing on the very experiment meant to measure the block layer. The one clean success on record, the post-034 Hispanic page with open colourable waves, carries `items.hair = 'thick wavy hair'` and therefore fired `hs-hair`. So what the ten runs measure is the *general* blocks losing nine times in ten; whether the dedicated block changes that is the open question, and D106's apparent test of it was confounded — when `hs-hair` first fired under 032 its value still contained "dark", the colour word D105 had not yet found. The "~90% failure rate" this sentence originally leaned on is withdrawn per the correction above; what remains true is that D106's investigations each sampled once against a spread of 8.5 points. **Any future hair claim needs about 8 points of effect to clear the noise, or several samples.** Two tools are committed for this: `scripts/ink-metrics.mjs`, which reports black fraction, black/white crossings per 1000px and median white run length — the last two being what separate an open section from hatching, which black fraction alone cannot do — and `scripts/contact-sheet.mjs`, which tiles a run so ten pages are judged against each other rather than against memory. |
| D112 | **At the block level the words are already right, and the pictures are the untested suspect.** Reading the twelve blocks on `hs-solo-portrait` against a 90% failure rate gives three things. **(1) Four blocks already say it.** `hs-figure-rendering`, `hs-hair`, `hs-texture-as-pattern` and `hs-output` each assert open white interiors, in different words, in the same prompt. Repetition is not the lever and a fifth statement will not help. **(2) Nothing in the layer is countable.** "A few large flowing sections", "large enough to colour comfortably" — every size instruction is qualitative, so the model has no target for how many sections hair divides into. A countable constraint is the one kind of instruction never tried here. **(3) `hs-line-quality` pulls against this element specifically.** It assigns weight by depth in scene — "confident heavy contours on figures and foreground objects, finer lines for background" — and hair is on the figure, so every curl contour is heavy. Weight is specified by depth and never by feature scale, which means the smaller the feature the harder that rule works against colourability; on tight curls the contours merge into the mass. **And the strongest lead is one D106 already dismissed.** Measured on their own hair regions, the two approved exemplars score **28.1%** and **27.1%** black with a 3–4px open run — *above the entire ten-run generated range of 17.2–25.7%*. The images handed to the model as the target are more filled than anything the model produced on its own; they are, measurably, examples of the failure the blocks exist to prevent. D106 ruled them out on a single byte-identical-prompt comparison, and against sd 2.7 a single sample cannot detect that effect, so the ruling does not hold. **Two properly powered experiments follow, ten runs each.** Arm B: `hs-hair` firing, which 037 makes possible by setting `items.hair` on the 16 Multiracial Solo portraits — the D111 baseline ran with that block absent. Arm C: the same prompt with `useReferences` off, against arm B, to settle the exemplar question on more than one sample. **Arm B was attempted and did not run: the OpenAI account returned `429 You have no credits remaining`** on all five calls, which cost nothing and produced nothing. Both arms are blocked on credit, not on code. |
| D113 | **Two of D103's starved axes are filled, and the first one was starved for a reason worth writing down.** `art_style` went from 1 of 180 coloring rows to 180 of 180 (038), `background_density` from 1 to 180 (039). **The style axis was not simply unfilled — it was unconstrained.** All seven style blocks are attached to all six templates, so the engine accepts any style on any page, and the combinations are not interchangeable: `Hispanic Spring 06` was generated as a Community Scene under Bold Minimal and under Botanical Line and both were rejected, the figures crude and unmarked, while Zentangle Pattern is "dense abstract pattern tiled across the whole page" and would take a portrait apart. So **all 96 figure pages get Editorial Scene**, which is what every approved page used, and the consequence is that D103 was partly misread: the variety axis does almost nothing for the figure pages, whose variety has to come from the briefs — which is where 031, 035 and 036 put it. **The axis lives on the 84 non-figure rows**, where there is no figure to protect and the style block is the main thing telling one page from another; those are assigned individually from what each brief already describes, six styles in use, and no season's three Quote pages share one. **039 closed a contradiction nobody had noticed.** `hs-comp-quote-page` requires an upright oval two-thirds the page wide "left completely blank with nothing crossing into it"; `hs-density-dense` requires that "the page is rich with real objects". Both are attached to the Quote page template, so the form would send both and the prompt would ask for a page that is full and two-thirds empty at once. This is §2.5's quieter cousin — not a negative summoning what it forbids, but two blocks that each make sense and cannot both be obeyed — and a check now stops the pairing being written again. Of D103's four axes that leaves `{{clean_date}}`, which is one VVS row and needs a per-order mechanism, and `{{environment}}`, which **D97 already resolved should stay empty** — the briefs carry their own setting. |
| D114 | **A generation now records what it was shown, and an Environment page is being shown two portraits.** D108 noted that `useReferences` was never written to the job; D106 had spent a controlled test on exactly that flag, and no run could be audited afterwards because nothing wrote it down. The reference selection now happens **before** the job row is inserted rather than after — what the model was shown is part of the request and belongs in the same row as the prompt — and `params_json` carries both `useReferences` as asked and `references` as the list of files actually supplied, which differ whenever a request asks for exemplars and none are usable. Verified without spending anything: the account is out of credit, the job row is written before the provider call, so a 429 still exercises the path. Those free failures also demonstrated D108's other half concretely — a **Community Scene** and an **Environment page** were each handed `hoodie-on-sofa-v2` and `walking-with-mug`, two close-up portraits of one seated man, as their style reference. For the four templates with `has_people = false` that is a category error rather than a judgement call; for Community Scenes, whether the portrait exemplars help or should be withheld is a real product choice and is left to Esoh. `reference_images.page_type` is populated on all four rows and the query still does not read it. |
| D115 | **The 56 pages without people get briefs, and a quote page was being asked to draw its own quote.** 040 finishes the coloring library: the Quote, Symbol, Decorative and Environment rows on the African American and Multiracial lines went from 88–106 characters to 248 and 239. **These pages had nowhere else to get their content.** A thin Solo portrait brief still generates something sane because the figure blocks, the hair value and the exemplars all speak; strip "Change and worth theme" from a Quote page and nothing remains but "a decorative arrangement around a clear open area". Each is written to the `art_style` and `background_density` 038 and 039 gave it — a Zentangle row briefed as tiled pattern, a Bold Minimal row as a few large shapes and a lot of nothing — because a brief that fights its own style block is how the two rejected Community Scenes happened. **Two contradictions were fixed rather than carried forward.** `hs-comp-quote-page` ends "Draw no letters, words or writing anywhere", since the quote is set afterwards by the print pipeline; `Multiracial Winter 11` was briefed as "bold lettering" and `Multiracial Summer 11` as "bold text space", both now rewritten to describe what surrounds the blank centre. And two of the five "bright" uses D105 left flagged for Esoh sit in rewritten rows and are simply gone — **that is a decision made in passing and it should be visible**; the other three stand. **Still open, found by the guard and deliberately not fixed:** `Hispanic Spring 09` and `Hispanic Spring 11` say "space in the center for the quote" and "so the quote feels like a quiet declaration". Naming the lettering inside a prompt that forbids drawing it is §2.5 in a milder key, but that is Esoh's revised text from 031 and is flagged rather than edited. The African American register survived the rewrite under check — "faith symbols" became praying hands and a cross, the braid still runs through the Spring cluster, the doves are in both places they were, and the church fellowship hall is still a church fellowship hall. **All 180 coloring rows now carry a brief, a style and a density; the shortest brief is 166 characters.** |
| D116 | **The exemplars carry the house style, and the hair was never as broken as the model said it was.** Twenty runs, $5.18, two arms identical but for `useReferences`, with `hs-hair` firing in both (037). **Arm C, exemplars off, is a different product.** Not flatter — different: bold simplified vector shapes, flat masses, minimal faces, a window reduced to a few thick strokes, `crossings` down to 36.3 from 48.8 because the whole page simplifies, not just the hair. Esoh's verdict on it was "too rigid and darkened", on every one of the ten. **D106 described this same change as "still filled, slightly flatter" from a single sample, and that is the clearest demonstration yet of what one sample is worth on this project.** So the exemplars stay: turning them off was never available as a fix, because it does not isolate hair — it replaces the book's entire look. **And the failure rate this investigation was chasing was not real.** Esoh reviewed the arm B and baseline sheets and judged the hair **acceptable**, noting that even the darkest two "were able to be colored even with the darkness". D111's "nine of ten filled" was the model's own eye substituted for the product owner's, and it is withdrawn there. **The measurement consequence is the part to keep**: `black %` counts ink, and ink quantity is not what Esoh is judging. A dark head whose curls read as separate closed shapes is colourable; a lighter one packed with hatching is not. `scripts/ink-metrics.mjs` reports `crossings` and `openrun` alongside black precisely because black alone could not tell those apart — but none of the three has been calibrated against a real accept/reject call. Until a set of pages Esoh has labelled exists, **no metric here should be treated as a verdict**, and the one intervention with a signal behind it is `hs-hair` firing: 0 of 10 under 17% black in the baseline against 3 of 10 with the block present, everything else held, which at n=10 is suggestive and not proven. |
| D117 | **A reference is only shown to a page that can use it.** D108 and D114 established that reference selection filtered on category alone while `reference_images.page_type` sat populated and unread, so all six Healing Seasons templates received the same two images. Both usable references are Solo portraits — two close-up views of one seated man — and **132 of the 180 coloring pages were being handed them regardless**, including the 84 Quote, Symbol, Decorative and Environment pages whose composition blocks open with "No people". A page forbidden to draw a figure was being shown a figure as the thing to imitate. Esoh's call, taken 2026-09-11: **match on whether the page has figures**, not on page type exactly. A Community Scene and a Solo portrait share how a figure is rendered, so the reference still teaches something true there and both keep it; the 84 pages with nobody on them get none. Matching page type strictly would have left five of six templates with no style reference at all, and there is no evidence yet on whether that helps or hurts them. A reference with a null `page_type` still applies everywhere, which is the shape a general style plate would take if one is added. **The deeper gap is unchanged and worth stating plainly: the library holds approved references for one page type out of six.** |
| D118 | **The season stops being a window, and the exemplar-season worry is probably unfounded.** Raised by Esoh from the contact sheets: the window with leaves "seems to be a set in stone ideal of an autumn scene which really isn't true at all". Correct about the block, and the cause was in two places, both of them the model's own work. **`hs-seasonal-restraint` offered the season two channels**, what the room is dressed with and what is visible through the window, and 033 then added "anything outside is seen through a window drawn as an enclosed frame" — written to stop leaves scattering across the wall plane, which it does, with the side effect of making the window compulsory in shape for any outdoor view. 041 gives the season five channels — what is worn, what is held or being done, what the room is dressed with, what is growing or cut, what is being eaten — each of them a thing outline can draw, which "the light" in the block's first version never was (§3.5). Window containment is kept but made conditional, and the clause against seasonal objects on every surface is kept because D107 showed it holding. **Second cause: two briefs asked for the window themselves**, `African American Fall 01` and `Multiracial Fall 01`, both written in 035 and 036 the same day — a block cannot argue with a subject line. 042 moves their season into a sweater, a blanket, apples, boots, a quilt, a lamp lit early. **The impression was also amplified by the experiment design**: all thirty pages Esoh reviewed were renders of `Multiracial Fall 01`, so every one had the window its own brief asked for; across the real Fall season only 2 of 15 rows on that line named one. **On season-matched references, the model's earlier claim that new ones are needed before Spring, Summer or Winter is withdrawn as unsupported.** Both exemplars are autumn, and since D117 they reach every figure page in all four seasons — but D107 established that the pumpkin in `hoodie-on-sofa-v2` is *not* taught, no page generated under the block has produced one, and D116 showed what the exemplars do transfer is rendering style, overwhelmingly. Content has not been transferring. The autumn in those images is content. So the expected answer is that a Summer page will not come back autumnal, and it costs about three generations to find out rather than a reference-shoot. |
| D119 | **Five pages, four page types, and the first quote page the project has ever made.** $1.40, with no `artStyle` or `density` passed so the item's own values from 038 and 039 had to drive it — they did. **The quote page is the result.** `African American Fall 09` came back as a botanical leaf-and-acorn border around a clean blank oval, no lettering, large colourable shapes — and it received **no style reference at all**, because D117's filter gives none to a page with no people. Five of six templates had never been generated; the one most likely to fail produced the best page of the run. **The community scene works too**, the first that has: `Multiracial Summer 05`, six adults in a circle outdoors, which vindicates 038's ruling that figure pages take Editorial Scene — the two rejected attempts on file were Bold Minimal and Botanical Line. **D118 is confirmed: no autumn contamination.** Both Summer pages came back as summer despite being handed two autumn exemplars, so the reference shoot the model earlier said was needed is not needed. Three defects found, all recorded in D120. |
| D120 | **Three fixes, and only the one aimed at the blocks was aimed at the right layer.** From D119: leaves had moved from the window to the floor (eight indoors on an environment page, ten on a porch), a window appeared floating in an outdoor scene, and `RECOVERY JOURHAL` was written — misspelt — on a journal. 043 addressed all three; six re-runs, two per page, settled which worked. **The outdoor window: fixed, 2 of 2.** 041 had said "where a page has one, it is seen through a window", which on a page with no indoors is an instruction to invent a wall; conditioning it on the page being set indoors removed it cleanly. **Leaf scatter: improved, not solved, 3 of 4.** The countable limit — "two or three across the whole page", D112's never-tried instruction shape — held on both Fall 01 renders at zero leaves and on one environment render at two, but the other environment render still put seven on the floorboards. **Stray lettering: the block rule failed 2 of 2 and the brief rewrite worked 2 of 2.** That is the finding. `African American Summer 01` brief said "a **recovery journal** open on her knees" and printed the title, twice, with `hs-output` explicitly requiring writing surfaces be drawn blank. `African American Fall 01` said "writing in **a journal**" and came back blank, twice, same run. **An abstract noun placed directly against a writing surface reads as the title printed on it** — the D105 shape again, a brief asking for something the medium renders literally, and the second time in two decisions that a block lost to a subject line (D118 was the first). 044 rewords twelve such rows so the abstract noun stays in the sentence and stops touching the object; "the one she keeps through recovery" produced a blank journal 2 of 2. The register D115 protected is intact — recovery, prayer, gratitude and affirmation all still appear, checked. **Two Hispanic rows were changed, which D115 declined to do**: that decision spared Esoh's 031 text where the risk was a guess, and this is a measured failure with a mechanism. |
| D121 | **The library gets its first exemplars of its own making, and "too cartoonish" turns out to be a missing reference.** Esoh's review of the D120 re-runs named three things. **(1) Two of the pages are exemplars.** The `African American Summer 01` pair generated after 044's brief rewrite — "drawn correctly and a great exemplar". 046 approves both. Until now the library held approved references for one page type and one season: two autumn Solo portraits of the same seated man. These are summer, outdoors, a different figure, and the first references the project has produced through its own corrected pipeline rather than shot ahead of it. D117's `limit 3` would have pushed `hoodie-on-sofa-v2` out silently; Esoh's call was to keep all four and raise the limit, so figure pages now see two seasons. **(2) The environment page is not a leaf problem.** "Too cartoonish" is the same drift D116 measured with exemplars off — and D117 gives an Environment page no reference at all, because every approved reference is a Solo portrait and the filter matches on whether the page has figures. A border survives that (the quote page in D119 was the best page of its run with no reference); a furnished room does not, being close enough to a portrait to need the house style. **Approved references exist for one page type out of six, and that is now a measured cost rather than a note.** Candidates are being generated for Esoh to approve. **(3) The random-object faults were all in the brief, and the brief was the model's.** "Why are there random boots next to her" — 042 wrote "her boots set to one side", which implies she took them off and never says so, so the model drew boots on her feet *and* a spare pair. "A porch step" is a threshold and the model drew both sides of it, porch underfoot and living room behind. A basket placed with no reason reads as a prop. 045 rewrites the row: in thick socks, boots left on the step below, the front door closed at her back, the mug where she can reach it. **The rule this yields — an object needs a reason to be where it is, and a setting needs a side of the door — is the third defect in two days that looked like a block problem and was a subject-line problem** (D118, D120). |
| D122 | **A block was removing colourable area in the name of colourability, and the library gets references for a second page type.** Esoh on the environment candidates: env-1 and env-2 are "a good basis to build from… because with a little coaxing I could add designs to the furniture and floor that would add personalization", and env-4 is good "despite it lacking in depth details such as the entire image looking flat". Both notes land on blocks that were right in intent and too broad in reach. **`hs-texture-as-pattern` read "Form is described by outline alone"** — written against tone describing form, which stays — and in doing so it also ruled out the rug with a border, the quilt pieced into blocks, the grain along a floorboard. **Decoration and shading are not the same thing**, and every motif the block was excluding is another closed shape to colour, so a rule written to protect colourability was deleting it. 047 separates them: form is still outline, and surfaces now carry the decoration a real object would have, drawn as further closed shapes each large enough to take a pencil. It also serves D96 — two rooms with different rugs are two different pages. **`hs-comp-environment-page` was four words**, "No people. The place itself is the subject.", which said nothing about the place having corners; it now asks for walls meeting, a floor running away, and objects in front of and behind one another, since §3.5 rules out the usual means of depth. The no-people clause is strengthened at the same time, because the Hispanic candidate put a woman in the chair on a page that already forbade one. **`reading-nook` and `winter-living-room` are approved**, the first references for any page type but Solo portrait, and explicitly as a basis rather than a finished look. D22's guard caught that both were still `draft` and had to be approved first. **Reference routing is narrowed to match.** D117 matched on `has_people`, so Environment references would also have reached Quote, Symbol and Decorative pages — and the best quote page yet was made with no reference at all. Selection now prefers an exact page-type match and only falls back across types for pages that have figures, where what a reference teaches is how a figure is rendered. Four of six page types still have no reference, which stays the open gap. |
| D123 | **The block was summoning the leaves it forbade, and §2.5 now has a fourth case.** 043 had added "Fallen leaves and other loose natural material belong outdoors on the ground, and do not collect on floors, furniture or surfaces indoors", and leaves went on collecting indoors in every environment render after it. That clause **names leaves twice**. 048 removed it and replaced it with a positive description of the floor; **zero leaves in six renders across two qualities.** §2.5's law was read as governing technique negatives, with content negatives permitted by name — but the permitting cases were about objects a page might reasonably hold, like a pumpkin on a shelf (D107, where the restraint genuinely works). It does not extend to the single object the model most associates with the season being forbidden by name. **A second cause was real too**: of the five channels 041 gave the season, *what is worn* and *what is held or being done* both require a person, so an Environment page had three channels rather than five — the same shape as D104's hair menus, a rule written while only figure pages were being generated. 048 names what carries a season in an empty room. **048 also overshot and 049 corrects it.** "The floor is clear, swept and ready to walk on" removed the rug as well as the debris, at both qualities, undoing exactly what D122 had just given Esoh; "clear" was doing two jobs. The floor is now "swept, carrying only what has been laid down on purpose — a rug with its border and motif, a mat by the door", and both test renders came back with a bordered rug and a clean floor. **A method note worth keeping: low quality could not answer this question.** A leaf on a patterned rug is the same size as the rug's own motifs and low quality blurs both into one mush; five low-quality renders were unreadable and wasted. Low quality is sound for large structural checks — is there a person, a window, lettering — and useless for small detail against similar detail. |
