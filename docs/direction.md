...
**Known limitation in the quote overlay**
- The type box is a rectangle; the reserved area the model draws is an oval. A long first or last line can therefore touch the pattern at the corners, since the box is widest where the oval is narrowest. The fix is to constrain each line's width by its height in the ellipse rather than using one width for the block — worth doing when a real quote actually breaks, not before.
