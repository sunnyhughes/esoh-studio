...
- **Quote pages cannot complete.** The prompt reserves the centre and forbids drawn letters (D23), but no overlay step exists. The font question is now **answered** (D58–D61); what remains is the code: `opentype.js` to turn a string into vector paths, `sharp` to composite the preview, and the chosen faces vendored into the repo so a build never depends on Google being reachable.
