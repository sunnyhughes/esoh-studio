 ...
 * page handed to the Solo Portrait template, a required field left empty. The
 * caller turns it into a 400; a batch run turns it into a skipped row with a
 * reason, which is the whole point of telling the two apart.
 */
...
/**
 * Generate one job: resolve the prompt, call the provider, persist the images
 * and close the books on cost (D9).
 *
 * This was the body of `POST /api/generate` until Stage E needed to run it
 * without an HTTP request in front of it. Nothing about the logic changed in
 * the move — the reference routing of D117, the transparency measurement of
 * D34 and the cost accounting of D9 are load-bearing and are not worth having
 * two copies of.
 */
export async function runGeneration(body: GenerateBody) {
  let jobId: string | null = null;

  try {
    if (!body.templateId) {
      throw new GenerationRefused("templateId is required.");
    }
    ...
    return { jobId, prompt, assets };
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);

    // The row is closed out here rather than by the caller, because the job id
    // only exists inside this scope and a job left `processing` for ever is
